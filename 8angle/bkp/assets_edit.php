...
$meta_title = "Edit Assets - 8angle | POS  ";
require('header.php');
require('left.php');
	...
	<section class="content">
		...
		<div class="box">
				...
				if(isset($_POST['EditAssets']))
					{
						...
						if($assets->update($id,$name,$amt,$reason,$paidby,$pay_date,$description,$status,$assets_category_id,$name_on_check,$bank_name,$check_or_account_number))
						{
							$inserted ="Asset was Edited/Updated successfully";
						}
						else
						{
							$failure = "Error while Editing/Updating Asset!";
						}
					}
					if(isset($_GET['edit_id']) and is_numeric($_GET['edit_id']))
					{
						$id = ($_GET['edit_id']);
						$assetsDetails = $assets->getID($id); 				
					@	$name= $assetsDetails->name;
					@	$amt= $assetsDetails->amt;
					@	$reason= $assetsDetails->reason;
					@	$paidby= $assetsDetails->paidby;
					@	$pay_date= $assetsDetails->pay_date;
					@	$description= $assetsDetails->description;
					@	$status= $assetsDetails->status;
					@	$assets_category_id = ($assetsDetails->assets_category_id);
					@	$name_on_check = ($assetsDetails->name_on_check);
					@	$bank_name = ($assetsDetails->bank_name);
					@	$check_or_account_number = ($assetsDetails->check_or_account_number);
						if($paidby == "Cash") {
							$name_on_check = "";
							$bank_name = "";
							$check_or_account_number = "";
			...
			<form class="form-horizontal"   id="formAssets"  method='post' action="">
						...
						<label for="Assets category" class="col-sm-3 control-label">Asset Type</label>
							<div class="col-sm-4">
				...
				<div class="box-footer">
					<a href="<?php echo BASE_URL;?>assets.php" class="btn btn-default">Cancel</a>
						<button type="submit" name="EditAssets" class="btn btn-info pull-right" value="Edit">Edit Asset</button>
				</div>
								<!-- /.box-footer -->
			</form>				
			</div>
