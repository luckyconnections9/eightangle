...
$show_tax = ($_POST['show_tax']);
	...
	if($stmt_product->rowCount() >0) {
		...
		while ($row = $stmt_product->fetch(PDO::FETCH_OBJ)) 
		{
			...
			$row_array['inv_product_cost']      	= $row->buy_price;
			...
			<td><input type="text"  value="<?php echo $row->buy_price; ?>" class="cost  form-control" name="inv_product_cost[]"  id="inv_product_cost" /></td>
			<td style="display: <?php echo $tax_display;?>">
				...
				if($show_tax == "Enable") 
					...
					$amount_te = $row->buy_price + (($row->buy_price * $taxv) / 100);
					$total_amount_te = $row->buy_price;
					$total_tax = ($row->buy_price * $taxv) / 100;
					$grand_total = $total_amount_te + $total_tax;
					$cgst_amt = ($row->buy_price * $cgst) / 100;
