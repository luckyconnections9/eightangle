...
$meta_title = "Edit Product - 8angle | POS  ";
require('header.php');
require('left.php');
				...
				if(isset($_POST['EditProducts']))
					{
						...
						if($products->update($id,$category_id,$name,$hsn_id,$hsn_code,$code,$buy_price,$sell_price,$price,$tax_id,$status,$description,$product_type,$products_unit_id))
						{
							$inserted ="Product was updated successfully";
						}
						else
						{
							$failure = "Error while updating Product!";
						}
					}
					if(isset($_GET['edit_id']) and is_numeric($_GET['edit_id']))
					{
						$id = ($_GET['edit_id']);
						$productDetails = $products->getID($id); 
					@	$name= $productDetails->name;
					@	$category_id= $productDetails->category_id;
					@	$code= $productDetails->code;
					@	$hsn_id= $productDetails->hsn_id;
					@	$hsn_code= $productDetails->hsn_code;
					@	$buy_price= $productDetails->buy_price;
					...
					@	$products_unit_id= $productDetails->products_unit_id;
					}
					?>
					<?php
					if($inserted)
					...
					?>
			<form class="form-horizontal" name="editproduct" id="formProducts"  method='post' action="">
				<div class="box-body">
					<div class="col-sm-8">
					...
					<div class="box-footer">
						...
						<button type="submit" name="EditProducts" class="btn btn-info pull-right" value="Edit">Edit Product</button>
					</div>
						<!-- /.box-footer -->
			</form>
		</div>
