...
$meta_title = "Create Purchase Invoice - 8angle | POS  ";
$invoice_js = "purchase";
require('header.php');
require('left.php');
...
$invoice_number_value_display = "";
	...
	if($company_data->purchase_tax == "Enable") 
	{ 
...
$invoice_number_value = "";
	...
	$invoice_number_value_hidden = $pus_invoice->getInvoicenumber();
	
	if($invoice_number  == "AutoIncrement") 
	{
		$invoice_number_value_display ="";
		//$invoice_number_value = $pus_invoice->getInvoicenumber();
...
<input type="hidden" id="Show_Purchasetax" value="<?php echo $company_data->purchase_tax; ?>" >
...
<div class="content-wrapper">
	...
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li><a href="<?php echo BASE_URL;?>pus_invoice.php">Purchase Invoice</a></li>
			<li class="active">Create Purchase Invoice</li>
		</ol>
    </section>
	<section class="content">
		...
		<?php	
					...
					$ship_to =  $company_data->name;
					$pin =  $company_data->pin;
					$gst = $company_data->gst_number;
					...
					if(isset($_POST['AddInvoice']) or isset($_POST['AddInvoicecon']))
						{
							...
							$invoice_type = "Purchase Invoice"; $invoice_id ="";
							
							if($order_id = $pus_invoice->create($invoice_number_value,$invtype,$inv_user,$date,$inv_user_name,$customer_state,$inv_discount,$code,$same_as_billing_address,$address,$city,$state,$inv_comment,$invoice_reference,$due_period,$show_hsn,$show_tax,$billing_address_id,$ship_to,$gst,$pin,$invoice_type,$invoice_id))
							{
								for($i=0; $i< count($_POST['inv_product_name']); $i++)
								{
									...
									if(!empty($inv_product_name) AND !empty($inv_product_cost) AND is_numeric($inv_product_cost) AND !empty($inv_product_qty) AND is_numeric($inv_product_qty)) {
										...
										$vendor_order_item->bindparam(":order_id",$order_id,PDO::PARAM_INT);
										$vendor_order_item->bindparam(":inv_product_id",$inv_product_id,PDO::PARAM_INT);
										$vendor_order_item->bindparam(":inv_product_name",$inv_product_name,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":inv_product_type",$inv_product_type,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":inv_product_unit",$inv_product_unit,PDO::PARAM_INT);
										$vendor_order_item->bindparam(":inv_product_qty",$inv_product_qty,PDO::PARAM_INT);
										$vendor_order_item->bindparam(":tax_type",$tax_type,PDO::PARAM_INT);
										$vendor_order_item->bindparam(":tax_amount",$tax_amount,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":t_tax",$t_tax,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":inv_product_cost",$inv_product_cost,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":inv_product_discount",$inv_product_discount,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":inv_product_discount_type",$inv_product_discount_type,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":cgst_tax_percent",$cgst_tax_percent,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":sgst_tax_percent",$sgst_tax_percent,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":cgst_amount",$cgst_amount,PDO::PARAM_STR);
										$vendor_order_item->bindparam(":sgst_amount",$sgst_amount,PDO::PARAM_STR);
								...
								{
									$inserted ="Purchase Invoice was created successfully";
									$urll = 'pus_invoice_edit.php?edit_id='.$order_id.'&code='.$code.'&created=Y&invoice=saved';
									...
									{
										$urll = 'pus_invoice_add.php?created=Y&invoice=saved';
										echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$urll.'">'; 
										exit(); 
									}
								}
								...
								{
									$failure = "Error while creating Purchase invoice!";
								}
							}
							else
							{
						...
						?>
			<form class="form-horizontal" id="formPusinvoice" method='post' action="pus_invoice_add.php">
				...
				<div class="box-body" id="lc-invoice">
						...
						<div class="col-sm-12">
							...
							<div class="invoice_cust col-sm-4">
								<table>
									<tr>
										<th align="left">Purchase Invoice Number#</th><td  align="right">
										<input type="hidden" name="invoice_number" value="Manual"> 
										<div class="form-group">
											<div class="col-sm-12">
							...
							<div class="col-sm-12 table-responsive">
								<table id="items" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
									...
									?>
											...
											<?php $cols = 13; $cols2 = 9; $cols4 = 9; 
												...
												?>
													...
													<td>
														<div id="subtotal"><span style="float:left"><?php echo round($nettotal);?></div>
													</td>
													<td style="display: <?php echo $purchase_tax_display;?>">
									...
									<div class="row">
										...
										<div class="col-sm-3"> 
											<b>Reference (Purchase Invoice Number)</b>
