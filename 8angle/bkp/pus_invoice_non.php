					...
					<table id="dataTable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
						<thead>					
						 ...
						 <tbody>
							...
							$query = "SELECT * FROM `vendor_orders` WHERE  $searchquery `deleted` ='N' AND (`customer_id` = '0' OR `customer_id` = '') AND `company_id` = :company_id  AND `invoice_type` = 'Purchase Invoice' ORDER BY $sortby $sortorder";
							$params[':company_id'] = $company_id;	
							
							$records_per_page=10;
							$newquery = $pagination->paging($query,$records_per_page);
							$pus_invoice->dataviewnon($newquery,$params);
							?>
							<tr>
