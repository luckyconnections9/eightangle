...
$meta_title = "Edit Vendor - 8angle | POS  ";
require('header.php');
require('left.php');
		...
		<div class="box">
			<div class="box-body">
				...
				if(isset($_POST['EditVendor']))
					...
					if($vendors->update($id,$biz_name,$c_name,$gst_num,$phone,$email,$address,$city,$state,$status,$pin))
						{
							$inserted ="Vendor was Edited/Updated successfully";
						}
						else
						{
							$failure = "Error while Editing/Updating Vendor!";
						}
					}
					if(isset($_GET['edit_id']) and is_numeric($_GET['edit_id']))
					{
						$id = ($_GET['edit_id']);
						$vendorsDetails = $vendors->getID($id); 				
					@	$biz_name= $vendorsDetails->name;
					@	$c_name= $vendorsDetails->c_name;
					@	$gst_num= $vendorsDetails->gst_num;
					@	$phone= $vendorsDetails->phone;
					@	$email= $vendorsDetails->email;
					@	$address= $vendorsDetails->address;
					@	$city= html_entity_decode($vendorsDetails->city);
					@	$state= html_entity_decode($vendorsDetails->state);
					@	$status= html_entity_decode($vendorsDetails->status);
					@	$pin= html_entity_decode($vendorsDetails->pin);
					}
					?>
					<?php
					if($inserted)
				...
				<form class="form-horizontal" id="formVendors"  method='post' action="">
					...
					<div class="box-footer">
						<a href="<?php echo BASE_URL;?>vendors.php" class="btn btn-default">Cancel</a>
							<button type="submit" name="EditVendor" class="btn btn-info pull-right" value="Edit">Edit Vendor</button>
					</div>
									<!-- /.box-footer -->
				</form>				
			</div>
