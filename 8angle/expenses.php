...
$meta_title = "Manage Expenses - 8angle | POS  ";
require('header.php');
require('left.php');
			...
			<div class="box-header">
					<form action="" method="get" class="form-horizontal" >
						...
						<select class="form-control" name="sortby">
							...
							<option value="name" <?php if($sortby == "name") { echo "selected"; } ?> >Expense</option>
							<option value="cname" <?php if($sortby == "cname") { echo "selected"; } ?> >Expense Type</option>
				...
				<a href="expenses_add.php" class="btn btn-large btn-info col-sm-2"><i class="glyphicon glyphicon-plus"></i> Add Expense</a>
			</div>
			<div class="box-body">
				<div class="col-sm-12">
					<?php
					...
					<thead>					
					 ...
					 <tbody>
						...
						$query = "SELECT *,(SELECT name 
							...
							WHERE expenses.expenses_category_id = expenses_category.id)cname FROM `expenses` where $searchquery `company_id` = :company_id  AND `deleted` = 'N' ORDER BY $sortby $sortorder"; 
						$params[':company_id'] = $company_id;
							
						$records_per_page=20;
						$newquery = $pagination->paging($query,$records_per_page);
						$expenses->dataview($newquery,$params);
						?>
						<tr>
