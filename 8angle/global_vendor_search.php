...
if(!empty($user_input)) 
{
	...
	if($stmt_customer->rowCount()>0)
	{
		...
		{ 
			$city_data =$vendors->getCity($recResult->city);	
		?>
	...
	{ 
		echo "<b style='color:red'>No Vendor Found."; ?><a style="cursor: pointer;"
		  onclick="window.open('addvendorquick.php','',' scrollbars=yes,width=600, resizable=yes,directories=0,titlebar=0,toolbar=0,location=0,status=0,menubar=0').focus()"> Add new</a></b> <?php 
	}
}
else 
{
	echo "<b  style='color:red'>No Vendor Found."; ?><a style="cursor: pointer;"
  onclick="window.open('addvendorquick.php','',' scrollbars=yes,width=600, resizable=yes,directories=0,titlebar=0,toolbar=0,location=0,status=0,menubar=0').focus()"> Add new</a></b>
<?php } ?>