...
class invoiceClass
{
	...
	public function create($invoice_number_value,$invtype,$inv_user,$date,$inv_user_name,$customer_state,$inv_discount,$code,$same_as_billing_address,$address,$city,$state,$inv_comment,$invoice_reference,$due_period,$show_hsn,$show_tax,$billing_address_id,$ship_to,$gst,$pin,$invoice_type,$invoice_id)
	{
		try
		{
			$st = $this->db->prepare("SELECT * FROM `orders` WHERE invoice_number=:invoice_number_value  AND `company_id` = $this->company_id  AND `deleted` = 'N' ");
			$st->bindparam(":invoice_number_value",$invoice_number_value,PDO::PARAM_STR);
			$st->execute();
			$count=$st->rowCount();
			if($count<1)
			{
				$stmt = $this->db->prepare("INSERT INTO orders(invoice_number,invtype,customer_id,dt_created,contactname,customer_state
...
,:inv_discount,:code,:same_as_billing_address,:address,:city,:state,:inv_comment,:invoice_reference,:due_period,:show_hsn,:show_tax,$this->company_id,$this->user_id,:billing_address_id,:ship_to,:gst,:pin,:invoice_type,:invoice_id)");
	...
	{
		$stmt = $this->db->prepare("SELECT SUM(paid_amount) FROM `orders` WHERE `invoice_number` = :inv_id ");
	...
	{
		$stmt = $this->db->prepare("SELECT invoice_number,invoice_prefix,invoice_number_start FROM companies WHERE `id` = :company_id AND `deleted` = 'N' LIMIT 1");
		$stmt->bindparam(":company_id",$this->company_id,PDO::PARAM_INT);
		$stmt->execute();
		$editRow=$stmt->fetch(PDO::FETCH_OBJ);
		if(!empty($editRow->invoice_prefix)) {
			
			$st = $this->db->prepare("SELECT id,invoice_number FROM orders WHERE invoice_number LIKE '".$editRow->invoice_prefix."%'  AND `company_id` = $this->company_id  AND `deleted` = 'N' AND `invoice_type` = 'Sale Invoice' ORDER BY `id` DESC ");
			$st->bindparam(":name",$name,PDO::PARAM_STR);
			$st->execute();
			$count=$st->rowCount();
			if($count >= 1) {
				$Row=$st->fetch(PDO::FETCH_OBJ);
				$inv_num = $editRow->invoice_prefix. sprintf("%03d", $count+$editRow->invoice_number_start);
				$inv_num1 = $editRow->invoice_prefix. sprintf("%03d", $editRow->invoice_number_start);
				$last_num = str_replace($editRow->invoice_prefix,"",$Row->invoice_number);
				
				if( $inv_num1 == $Row->invoice_number ) 
					{
						return $editRow->invoice_prefix. sprintf("%03d", ($editRow->invoice_number_start+1));
					}
					else 
					{
						if($last_num < $editRow->invoice_number_start) {
							return $editRow->invoice_prefix. sprintf("%03d",  $editRow->invoice_number_start);
						}
						if($last_num >= $editRow->invoice_number_start) {
							return $editRow->invoice_prefix. sprintf("%03d",  $last_num+1);
						}
					}
	...
	public function update_balance($order_id,$nettotal,$tax,$discount,$total,$balance,$freight,$invoice_paid,$paid_amount)
	{
		try
		{
			if(empty(invoiceClass::getID($order_id))) { return false; } 

				$stmt=$this->db->prepare("UPDATE orders SET nettotal=:nettotal,tax=:tax, discount=:discount, total=:total ,balance=:balance, freight=:freight,invoice_paid=:invoice_paid, paid_amount=:paid_amount WHERE id=:order_id ");
	...
	public function update_payment_details($order_id,$paid_by,$paid_date,$name_on_check,$check_or_account_number,$bank_name,$remarks)
	{
		try
		{
			if(empty(invoiceClass::getID($order_id))) { return false; } 
				...
				$stmt=$this->db->prepare("UPDATE orders SET paid_by=:paid_by,paid_date=:paid_date, account_name=:name_on_check, account_number=:check_or_account_number ,bank_name=:bank_name, remarks=:remarks WHERE id=:order_id ");
	...
	public function delete($id)
	{
		try
		{
			if(empty(invoiceClass::getID($id))) { return false; }
			
				$stmt = $this->db->prepare("UPDATE orders SET deleted='Y' WHERE id=:id  AND `company_id` = $this->company_id ");
				$stmt->bindparam(":id",$id,PDO::PARAM_INT);
				$stmt->execute();
				$stmtstock = $this->db->prepare("UPDATE stock SET deleted='Y' WHERE order_id=:id AND `type` = 'OUT'  AND `company_id` = $this->company_id ");
	...
	public function dataview($query,$params)
	{
		...
		if($stmt->rowCount()>0)
		{
			...
			{
				$cust_data = invoiceClass::getCustomer($row->customer_id);	
				$comp_data = invoiceClass::getCompany($this->company_id);	
			?>
				<tr>
			...
			<td><?php if($row->customer_id) { print($row->cname); } else { print($row->contactname); } ?></td>
					...
					<td><?php if($row->state == $comp_data->state)  print(html_entity_decode($row->cgst)); ?></td>
					<td><?php if($row->state == $comp_data->state)  print(html_entity_decode($row->sgst)); ?></td>
					<td><?php if($row->state != $comp_data->state) print(html_entity_decode($row->igst)); ?></td>
					...
					<td  align="center">
						<a href="invoice_edit.php?edit_id=<?php print($row->id); ?>&code=<?php print($row->code); ?>"><i class="glyphicon glyphicon-edit"></i></a>
					</td>
					<td align="center">
						<a href="invoice_view.php?print_id=<?php print($row->id); ?>" data-toggle="modal" data-target="#myModal<?php print($row->id); ?>" title="View Invoice"><i class="fa fa-eye"></i></a>
					</td>
					<td align="center">
						<a href="printsaleinvoice.php?print_id=<?php print($row->id); ?>" target="_blank" ><i class="fa fa-print"></i></a>
					</td>
					<td align="center">
						<a title="Create Receipt" href="receivable_add.php?inv_id=<?php print($row->id); ?>" ><i class="fa fa-file-text-o"></i></a>
					</td>
					<td align="center">
					<?php if($i == 1) { ?>
						<a href="invoice_delete.php?delete_id=<?php print($row->id); ?>" onClick="return(confirm('Are you sure?'))"><i class="glyphicon glyphicon-remove-circle"></i></a>
					<?php } ?>
	...
	public function dataviewnon($query,$params)
	{
		...
		if($stmt->rowCount()>0)
		{
			...
			<td><?php if($row->customer_id) { print($cust_data->name); } else { print($row->contactname); } ?></td>
					...
					<td align="center">
						<a title="Create Receipt" href="receivable_add.php?inv_id=<?php print($row->id); ?>" ><i class="fa fa-file-text-o"></td>
					<td align="center">
						<a href="printsaleinvoice.php?print_id=<?php print($row->id); ?>" target="_blank" ><i class="fa fa-print"></i></a>
					</td>
					<td align="center">
