	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header">
				<div class="col-sm-12">
				<a href="invoice_add.php" class="btn btn-large btn-info pull-right"><i class="glyphicon glyphicon-plus"></i> Create Sale Invoice</a>
				</div>
			...
			<div class="box-body">
				<div class="col-sm-12">
					...
					$params = array();
							...
							FROM customers
							WHERE orders.customer_id = customers.id)cname,(SELECT SUM(tax) 
							FROM order_item
							WHERE orders.id = order_item.order_id)igst,(SELECT SUM(cgst_amount) 
							FROM order_item
							WHERE orders.id = order_item.order_id)cgst,(SELECT SUM(sgst_amount) 
							FROM order_item
							WHERE orders.id = order_item.order_id)sgst  FROM `orders` WHERE  $searchquery `deleted` ='N' AND `company_id` = :company_id AND `invoice_type` = 'Sale Invoice' ORDER BY $sortby $sortorder";
						...
						$ttl_stmt = getDB()->prepare("SELECT *, (SELECT SUM(tax) 
							...
							WHERE orders.id = order_item.order_id)cgst,(SELECT SUM(sgst_amount) 
							FROM order_item
							WHERE orders.id = order_item.order_id)sgst  FROM `orders` WHERE  $searchquery `deleted` ='N' AND `company_id` = :company_id AND `invoice_type` = 'Sale Invoice' ");
						...
						if($ttl_stmt->rowCount()>0)
						{
							while($ttl_d=$ttl_stmt->fetch(PDO::FETCH_OBJ)) {
								$comp_data = $invoice->getCompany($company_id);
								if($ttl_d->state == $comp_data->state) { $ttl_cgst = $ttl_cgst +  $ttl_d->cgst;  $ttl_sgst = $ttl_sgst +  $ttl_d->sgst; }
								if($ttl_d->state != $comp_data->state) { $ttl_igst = $ttl_igst +  $ttl_d->igst;  }
				...
				<div class="col-sm-12">
					<div class="well col-sm-3"><b>Total Sale : </b> <?php echo num($ttl);?></div>
