...
$meta_title = "Create Purchase Credit/Debit Receipt - 8angle | POS  ";
$invoice_js = "purchase";
require('header.php');
require('left.php');
...
$invoice_number_value_display = "";
		...
		$invoice_number_value = $receivable->getDCpurchaseReceiptnumber();
	
		$inv_id = "";
		if(isset($_GET['inv_id']) and is_numeric($_GET['inv_id'])) {
			$inv_id = mysql_real_escape_string($_GET['inv_id']);
		}
?>
...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>
			Create Purchase Credit/Debit Receipt
		   <small></small>
		</h1>
		<ol class="breadcrumb">
			<li><a href="<?php echo BASE_URL;?>"><i class="fa fa-dashboard"></i> Dashboard</a></li>
			<li><a href="<?php echo BASE_URL;?>receipts.php?receipt=Purchase">Purchase Sale Credit/Debit</a></li>
			<li class="active">Create Purchase Credit/Debit Receipt</li>
		</ol>
    </section>
	<section class="content">
		...
		<?php	
					$failure = "";
					$inserted = ""; $paid_amount = 0;
					 $invoice_type ="";
						?>
						<?php	
						if(!$pus_invoice->getID($inv_id)) {
							?>
							<div class="row">
								<div class="col-sm-12"  style="padding:4%">
									<div class="col-sm-3">
									<b>Invoice Number:</b>
											<input autocomplete="off" type="text"   class="form-control input-sm"  onkeyup="load_pus_invoice(this.value);" name="inv" value="<?php echo $inv_id;?>" placeholder="INV001" />
									</div>	
						...
						{
							$invoice_data = $pus_invoice->getID($inv_id);
							$invoice_no = mysql_real_escape_string($invoice_data->invoice_number);
							...
							if($vendorsDetails = $vendors->getID($inv_user)) {
								
								$vendorsDetails = $vendors->getID($inv_user); 
								$name= $vendorsDetails->name;	
								$customer_gst= $vendorsDetails->gst_num;
								$customer_phone= $vendorsDetails->phone;
								$customer_email= $vendorsDetails->email;
								$customer_address = $vendorsDetails->address;
								$customer_city = $vendorsDetails->city;
								$customer_state = $vendorsDetails->state;
					...
					if(isset($_POST['EditInvoice']) or isset($_POST['EditInvoicenew']))
						{
							...
							$invoice_type = mysql_real_escape_string($_POST['invoice_type']);
							...
							if($order_id = $pus_invoice->create($invoice_number_value,$invtype,$inv_user,$date,$inv_user_name,$customer_state,$inv_discount,$code,$same_as_billing_address,$address,$city,$state,$inv_comment,$invoice_reference,$due_period,$show_hsn,$show_tax,$billing_address_id,$ship_to,$gst,$pin,$invoice_type,$inv_id))
							{
								...
								if($pus_invoice->update_balance($order_id,$nettotal,$tax,$discount,$total,$balance,$freight,$invoice_paid,$paid_amount))
								{
									...
									$urll = 'payable_edit.php?created=Y&receipt=saved&edit_id='.$order_id;
			...
			<form class="form-horizontal" id="formPusinvoice" method='post' action="payable_add.php?inv_id=<?php echo $inv_id;?>&code=<?php echo $code;?>">
				...
				<div class="box-body" id="lc-invoice">
						<div class="col-sm-12">
							<span class="col-sm-6" style="text-align:left">
								<h4>	Create:&nbsp;
									<input checked type="radio" name="invoice_type" <?php if($invoice_type  == "Debit Receipt") { echo "checked";}?> value="Debit Receipt" required /> Debit Receipt &nbsp;&nbsp;&nbsp;&nbsp;
									<input type="radio" name="invoice_type" <?php if($invoice_type  == "Credit Receipt") { echo "checked";}?> value="Credit Receipt" required /> Credit Receipt
						...
						<div class="col-sm-12">
							...
							<div class="invoice_cust_shipping_address col-sm-4" style="opacity:0;">
								...
								<span id="showShippingAddress" >
									<a href="invoice_addresses.php?customer=0&type=shipping" data-toggle="modal" data-target="#shippingAddress" title="" data-refresh="true"><i class="fa fa-plus">Add Shipping Address</i></a><div id="shippingAddress" class="modal fade"><div class="modal-content"></div><div class="modal-footer"><button type="button" class="btn btn-outline" data-dismiss="modal">Close</button></div></div>
								</span>
							</div>
							<div class="invoice_cust col-sm-4">
								<table>
									<tr>
										<th align="left">Receipt No.</th><td  align="right">
										<div class="form-group">
											<div class="col-sm-12">
												<input autocomplete="off" style="text-transform: uppercase"  class="form-control input-sm" id="invoice_number_value"  name="invoice_number_value" type="text" value="<?php echo $invoice_number_value;?>" <?php echo $invoice_number_value_display;?>  required />
											</div>	
										</div>
										</td>
									</tr>
							...
							<div class="col-sm-3 well">
								<table>
									...
									<tr>
										...
										<td  align="right">
										<?php
											$dr = getDB()->prepare("SELECT SUM(balance)  AS debit ,SUM(invoice_discount) AS dr_invoice_discount ,SUM(freight) AS dr_freight FROM `vendor_orders` WHERE `deleted` ='N' AND `company_id` = $company_id AND `invoice_type` = 'Debit Receipt' AND `invoice_id` = '$inv_id' ");
											$dr->execute();
											$debit = 0;
									...
									<tr>
										...
										<td  align="right">
										<?php
											$cr = getDB()->prepare("SELECT SUM(balance)  AS credit ,SUM(invoice_discount) AS cr_invoice_discount ,SUM(freight) AS cr_freight , GROUP_CONCAT(id SEPARATOR ', ') AS idss FROM `vendor_orders` WHERE `deleted` ='N' AND `company_id` = $company_id AND `invoice_type` = 'Credit Receipt' AND `invoice_id` = '$inv_id' ");
											$cr->execute();
											$cids = NULL;
											$credit =   0;
									...
									<tr>
										...
										<td  align="right"><b><?php echo $cur;?></b>  <span class="due"><?php echo ($balance - ($debit - $credit));?></span></td>
									</tr>
								</table>
							</div>
									...
									<?php } ?>
									   <tr>
										  <?php $cols = 13; $cols2 = 9; $cols4 = 9; 
											...
											?>
												<td colspan="<?php echo $cols4; ?>"><b>Total:</b></td>
												<td>
										...
										<tr>
											<td colspan="<?php echo $cols2; ?>" rowspan="3" class="blank" vertical-align="middle">
												...
												<?php if($company_data->state == $customer_state) { ?>
											  ...
											  <td class="total-value" colspan="2">
												<input type="text" class="form-control" size="32" name="inv_discount" id="discount" value="<?php echo num(($inv_discount - $dr_invoice_discount) + $cr_invoice_discount);?>" />
											  </td>
										</tr>
										<tr>
											  <td colspan="2" class="total-line">Freight</td>
											  <td class="total-value" colspan="2"><input type="text" class="form-control" size="32" name="freight" id="freight" value="<?php echo num(($freight - $dr_freight) + $cr_freight);?>" /></td>
										</tr>
										<tr>
					...
					<div class="box-footer">
						<a href="<?php echo BASE_URL;?>pus_invoice.php" onclick="return confirm('Are you sure you want to close this page')" class="btn btn-default">Cancel</a>
						
						<button type="submit" name="EditInvoicenew"  onclick="return confirm('You are about to create new receipt. Are you sure?')"  class="btn btn-info pull-right" style="margin-right:30px" value="Edit">New</button>
