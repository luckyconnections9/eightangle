...
if($redirect2 == 4) { $urll = 'login.php';  echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$urll.'">'; exit();} 
elseif($redirect2 == 3) {
}
else {
	$urll = 'register.php';  echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$urll.'">'; exit();
}
if($userClass->getCount() == true) 
{ $urll = 'login.php';  $myFile2 = 'includes/data/txt/register.txt';
		if(!is_file($myFile2)) {
			file_put_contents($myFile2,4);
		} else {
			file_put_contents($myFile2,4);
		} 
		$stmt1 = getDB()->prepare("UPDATE `settings` SET `value` = '4' WHERE `slug` = 'REGISTER' "); 
		$stmt1->execute();
